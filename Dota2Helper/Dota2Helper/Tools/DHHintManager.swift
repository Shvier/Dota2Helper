...
class DHHintManager: NSObject {
    ...
    let hintResouce: NSArray = [
        ["name": "snk", "hint": "请等待网络重生，如果...蓝量还够的话"],
        ["name": "sk", "hint": "电缆已被掘地穿刺挖断，什么？还有地震"],
        ["name": "fv", "hint": "似乎到了另一个时空维度"],
        ["name": "od", "hint": "该设备已被星体禁锢，无法与外界通信"],
        ["name": "sil", "hint": "网络通信已被沉默术士禁言，跟他谈谈吧，如果...他会谈话的话"]
    ]

    func getHint() -> HintData {
        let randIndex: Int = Int(arc4random_uniform(UInt32(hintResouce.count)))
        let hintDict: NSDictionary = hintResouce[randIndex] as! NSDictionary
